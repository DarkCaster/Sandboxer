...
if test -z "$ipc_file"; then
  ...
  xpra attach --socket-dir="$base_dir/control/xpra" --compressors= --compress=0 --encoding=rgb --speed=100 --min-speed=100 --quality=100 --min-quality=100 --tray=$enable_tray &
else
  1>"$base_dir/xpra-client.stdout" 2>"$base_dir/xpra-client.stderr" \
  xpra attach --socket-dir="$base_dir/control/xpra" --mmap="$ipc_file" --compressors= --compress=0 --encoding=rgb --speed=100 --min-speed=100 --quality=100 --min-quality=100 --tray=$enable_tray &
fi
xpra_pid="$!"
